...
def notification(request):
    ...
    permission_event_ids = Permission.objects.values_list('Event_id', flat=True)
    # Get Events that are not in the Permission table
    pending_events = Event.objects.exclude(id__in=permission_event_ids)

    # Get approved events for non-superusers
    if not current_user.is_superuser:
        approved_event_ids = Permission.objects.filter(status=approved_status).values_list('Event_id', flat=True)
        try:
            faculty = Faculty.objects.get(user=current_user)
            user_dept_club = faculty.dept_club
            approved_events = Event.objects.filter(id__in=approved_event_ids, dept_club=user_dept_club)
        except Faculty.DoesNotExist:
